...
## 🚨 ВАЖНО: ПРАВИЛА ДЛЯ AI-АГЕНТОВ (CRITICAL RULES FOR AI)
1. **Язык общения:** Все AI-агенты (включая Copilot, Cursor, Gemini и других) **ОБЯЗАНЫ** общаться с пользователем **ИСКЛЮЧИТЕЛЬНО на русском языке**. Все ответы, пояснения, комментарии к коду и текстовые сообщения должны быть на русском.
2. **Актуализация документации:** При внесении любых изменений в проект агенты обязаны поддерживать файлы `README.md` и `AGENTS_CONTRACT.md` в актуальном состоянии.
