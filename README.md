...
**Discord-бот и веб-панель (Dashboard) для комплексного управления семьёй в игре GTA V RP (Majestic RP).**

Этот проект автоматизирует рутинные задачи руководства (лидеров, заместителей, хай-рангов), такие как набор новых участников, управление мероприятиями (ВЗЗ, Капт, MCL), контроль активности состава и отслеживание статуса AFK.

## 🚨 ВАЖНО: ПРАВИЛА КОММУНИКАЦИИ И ДЛЯ AI-АГЕНТОВ
1. **Язык общения:** Все AI-агенты (включая Copilot, Cursor, Gemini и др.) **ОБЯЗАНЫ** общаться с пользователем **ИСКЛЮЧИТЕЛЬНО на русском языке**. Все ответы, пояснения и текстовые сообщения в чате должны быть на русском.
2. **Актуализация документации:** При внесении любых изменений в функционал или архитектуру, агенты постоянно обязаны поддерживать файлы `README.md` и `AGENTS_CONTRACT.md` актуальными, изучая текущие файлы проекта.

---

## 🚀 Основной функционал (Подробно)

1. **Система заявок (Тикеты):**
   - Пользователи подают заявки прямо в Discord через красивые embed-панели и модальные окна.
   - Администраторы обрабатывают анкеты в веб-панели, меняют статусы (Ожидает, На обзвоне, Принят, Отклонен).
   - Бот автоматически выдает системные роли в Discord и меняет никнеймы при успешном принятии кандидата.
2. **Система мероприятий (Events):**
   - Удобное создание событий (MCL, ВЗЗ, Capt) с авто-публикацией в настроенный канал.
   - Запись участников через интерактивные кнопки под сообщением с событием (автоматически формируются основной и резервный списки с отображением Discord-аватаров и ролей).
   - Выбор карт и настройка групп для турниров.
3. **Контроль онлайна и AFK:**
   - Бот периодически стягивает и отображает актуальный онлайн серверов Majestic.
   - Члены семьи могут брать AFK через бота (с указанием причины и времени), а администраторы следят за просроченными AFK через веб-дашборд.
4. **Активность (Activity Threads):**
   - Бот создает индивидуальные ветки (threads) на форуме Discord для каждого члена семьи.
   - Участники в один клик загружают скриншоты своей активности в DM бота. Они заливаются в их индивидуальные ветки и агрегируются в веб-панели.
5. **Гибкая система прав (Access & Roles):**
   - Настройка гранулярных доступов к страницам веб-панели и отдельным функциям бота. Полноценная интеграция с Discord Role ID.
...
- `EVENT_MCL_CHANNEL_ID` (канал для списков MCL/ВЗЗ)
- `EVENT_CAPT_CHANNEL_ID` (канал для списков Капт)
...
- `TICKETS_CHANNEL_ID`, `TICKETS_MESSAGE_ID`
- `EVENTS_CHANNEL_ID`, `EVENTS_MESSAGE_ID`
- `EVENT_MCL_CHANNEL_ID`, `EVENT_CAPT_CHANNEL_ID`
- `ONLINE_CHANNEL_ID`, `ONLINE_MESSAGE_ID`
- `AFK_CHANNEL_ID`, `AFK_MESSAGE_ID`
- `ACTIVITY_FORUM_CHANNEL_ID`
...
## 💡 Идеи для улучшения функционала (Roadmap)

В рамках дальнейшего развития проекта планируется внедрить следующие крутые фичи:

1. **Интеграция ИИ для обработки заявок (AI Screening):**
   - Автоматический анализ первичных заявок кандидатов на основе ключевых слов, возраста и опыта.
   - Генерация короткого саммари-портрета кандидата для ускорения работы проводящего обзвон.
2. **Продвинутая статистика и аналитика (Analytics Dashboard):**
   - Детальные графики активности (посещение мероприятий, общие часы онлайна, количество загруженных скриншотов).
   - Авто-формирование "Доски почета" или "Топ-10 активных за неделю" для поощрения игроков.
3. **Система достижений и геймификации (Achievements):**
   - Выдача кастомных ачивок на сайте или ролей в дискорде участникам (например, "Посетил 10 ВЗЗ подряд", "Ветеран семьи").
4. **Резервирование и логирование (Audit Logs & Webhooks):**
   - Отправка уведомлений о важных действиях администрации веб-панели (кик игрока, снятие роли, изменение системных настроек) в отдельный скрытый канал логов Discord.
   - Интеграция с Telegram для оповещения хай-рангов о критических изменениях в семье.
5. **Генерация OpenAPI/Swagger:**
   - Добавить авто-генерацию документации OpenAPI по Zod-схемам в `src/backend` (и использовать её в CI/CD).
6. **Укрепление “Contract Tests” и Task Scheduler:**
   - Переход от обычных `setInterval` на полновесную очередь задач (например, BullMQ) для стабильной работы кронов.
   - Введение строгих тестов для `customId` префиксов и маршрутов IPC.
7. **Система Rate-limiting и наблюдаемость:**
   - correlation-id для HTTP/IPC/WS.
   - Единый формат логов Pino для бэкенда и Discord.js логгера.
8. **Введение “system_settings schema”:**
   - Строгий валидатор ключей + миграции значений.
   - Автогенерация документации настроечных ключей для фронтенда.
...
### Вариант A: Full wipe (полный сброс) и установка нового бота
Если вы хотите полностью удалить все данные и привязать нового Discord-бота (новый токен, новая база), выполните эти шаги строго по порядку:
...
2. Удалите volume базы данных и кэша (это безвозвратно удалит старую БД):
```bash
docker volume rm kingsizebot_postgres_data kingsizebot_redis_data
```
*(Имена volumes могут отличаться. Если не сработало, посмотрите список через `docker volume ls`)*

3. Обновите файл `.env` своими новыми данными:
   - `DISCORD_TOKEN` — токен нового бота
   - `DISCORD_CLIENT_ID` — Client ID нового приложения
   - `DISCORD_CLIENT_SECRET` — Client Secret
   - `BOT_OWNER_ID` — ваш Discord ID

4. Запустите всё заново (с пересборкой):
```bash
docker compose up -d --build
```

5. Дождитесь, пока PostgreSQL полностью запустится, и перезапустите бэкенд чтобы миграции гарантированно накатились:
```bash
# Подождите ~10 секунд после запуска, затем:
docker compose restart backend
```

6. Убедитесь, что миграции прошли (в логах должно быть `Changes applied`):
```bash
docker compose logs backend --tail 10
```
*Если видите ошибку `relation "users" does not exist` — повторите шаг 5.*

7. Установите базовые роли в базу:
```bash
docker compose exec -T backend npx tsx src/db/seed.ts
```

8. Настройте `system_settings`:
   - Откройте веб-админку и авторизуйтесь через Discord (обязательно пропишите Redirect URLs в портале разработчиков Discord).
   - Зайдите в раздел «Настройки сервера» и заново введите `GUILD_ID`, ID каналов (заявки, онлайн, AFK) и прочее.
   - Сразу после этого бот заново создаст все embed-панели в Discord.
